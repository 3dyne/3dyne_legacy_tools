...
void SetupField( char *plane_name, char *node_name )
{
	...
	if ( !planehm )
		Error( "can't load field plane class.\n" );

	nodehm = NewHManagerLoadClass( node_name );
	if ( !nodehm )
		Error( "can't load field node class.\n" );
...
//		texobj = Misc_GenTexture_TGA_8888( ART_PATH "fx/blip2.tga" );
		texobj = Misc_GenTexture_TGA_8888( ART_PATH "fx/smoke_faces1.tga" );
