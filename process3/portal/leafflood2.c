...
static fp_t		flooded_volume = 0.0;
...
#if 0
fp_t CalcPortalArea( hobj_t *portal )
{
	polygon_t	*p;
	fp_t		area;
	int		i, num;
...
fp_t CalcLeafVolume( hobj_t *leaf )
{
	fp_t		area;
...
bool_t FloodThroughLeafsRecursive( hobj_t *leaf, hmanager_t *portalhm, hmanager_t *nodehm )
{
	...
	if ( contents == 0 || ( contents & flood_contents ) )
	{
		flood_num++;
		pair = NewHPair2( "string", "flood_state", flood_text );
		InsertHPair( leaf, pair );

		// if it got a volume add it to flooded_volume
		pair = FindHPair( leaf, "volume" );
		if ( pair )
		{
			fp_t		volume;
			HPairCastToFloat_safe( &volume, pair );
			flooded_volume+=volume;
		}
	}
	else
	{
