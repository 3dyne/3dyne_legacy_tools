...
ccmap3_t * G_NewCCMap3( int hashsize, int clustersize, int cellbytes )
{
	...
	if ( i == 32 )
		Error( "G_NewCCMap3: invalid hash table size\n ");

	size = (int)&(((ccmap3_t *)0)->hash[hashsize]);
...
ccluster_t * G_NewCCluster( int datasize )
{
	...
	size = (int)&(((ccluster_t *)0)->cdata[datasize]);
...
ccmap3_t * G_LoadCCMap3Binary( char *name )
{
	...
	{
		return NULL;
//		Error( "G_LoadCCMap3Binary: can't open file\n" );
	}
