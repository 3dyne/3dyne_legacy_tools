...
//	vec3d_t		max = {4608,192,0};
	...
	if ( update )
	{
		...
		R_FillVolumeCache( cache, a_map->volume_ccmap3, min, max );
		R_FillVolumeCache( cache2, a_map->field_ccmap3, min, max );
...
void RunTestSmoke( map3_cache_t *cache, map3_cache_t *cache2, bool_t reset )
{
	...
	if ( count == 0 )
	{
		...
		texobj = Misc_GenTexture_TGA_8888( ART_PATH "/fx/blip.tga" );
