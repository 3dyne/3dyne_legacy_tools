...
void DrawAll( visleaf_t *list )
{
	...
	GLD_BeginList( gldbg, "world", "line" );
	GLD_Color3f( gldbg, 1, 1, 1 );
...
int main( int argc, char *argv[] )
{
	...
	printf( "===== pvs_through - build potential visibility set for visleafs =====\n" );
	SetCmdArgs( argc, argv );

	in_visleaf_name = GetCmdOpt2( "-i" );
	in_plane_name = GetCmdOpt2( "-pl" );
	out_visleaf_name = GetCmdOpt2( "-o" );
	debug_visleaf_name = GetCmdOpt2( "-debug" );
//	debug_portal_name = GetCmdOpt2( "
	...
	if ( debug_visleaf_name )
	{
		printf( " debug visleaf enabled for: %s\n", debug_visleaf_name );
	}
...
#ifdef GLDBG
	gldbg = GLD_BeginSession( "vis.gldbg" );
	...
	if ( !CheckCmdSwitch2( "--trivial-only" ) )
		ComplexReject( visleaflist, debug_visleaf_name ); 

#ifdef GLDBG
	GLD_EndSession( gldbg );
#endif
