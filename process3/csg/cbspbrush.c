...
cbspbrush_t * NewBrush( int surfnum )
{
	...
	size = (size_t)&(((cbspbrush_t *)0)->surfaces[surfnum]);
...
cbspbrush_t * CopyBrush( cbspbrush_t *in )
{
	...
	size = (size_t)&(((cbspbrush_t *)0)->surfaces[in->surfacenum]);
...
void CSG_SplitBrush_new( cbspbrush_t *in, cplane_t *plane, cbspbrush_t **front, cbspbrush_t **back )
{
	...
	*front = *back = NULL;
	...
	if ( b->surfacenum < 4 )
	{
		printf( "no back %d ( split brush '%s' )\n", b->surfacenum, in->original->name );
		printf( "split plane is %f, (%f %f %f)\n", plane->dist, plane->norm[0], plane->norm[1], plane->norm[2] );
		FreeBrush( b );
		b = NULL;
	}
	if ( f->surfacenum < 4 )
	{
		printf( "no front %d ( split brush '%s' )\n", f->surfacenum, in->original->name  );
                printf( "split plane is %f, (%f %f %f)\n", plane->dist, plane->norm[0], plane->norm[1], plane->norm[2] );
