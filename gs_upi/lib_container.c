...
void U_DLNInsertList( u_dln_t *prev, u_dln_t *next, u_dln_t *insert_left, u_dln_t *insert_right )
{
	prev->next = insert_left;
	insert_left->prev = prev;
	next->prev = insert_right;
	insert_right->next = next;
}
...
/*
  ==============================
  U_ListInsertListAtHead

  ==============================
*/
void U_ListInsertListAtHead( u_list_t *list, u_list_t *insert )
{
	if ( insert->head->next == insert->tail )
		return;
	
	U_DLNInsertList( list->head, list->head->next, insert->head->next, insert->tail->prev );	

	insert->head->next = insert->tail;
	insert->tail->prev = insert->head;
}
...
void * U_ListBuildObjPtrArray( u_list_t *list, int *out_len )
{
	...
	*out_len = len;
	...
	base = NEWBYTES( len*sizeof( maddr_t) );
...
/*
  ...
  ==============================
*/
void U_CleanUpRing( u_ring_t *ring, void (*cleanup_func)(void *obj) )
{
	u_dln_t		*next, *n;
...
void * U_RingGetCurrent( u_ring_t *ring )
{
	...
	return ring->current->objptr;
}

/*
  ==============================
  U_RingGetCurrentPrev

  ==============================
*/
void * U_RingGetCurrentPrev( u_ring_t *ring )
{
	if ( !ring->current )
		return NULL;
	return ring->current->prev->objptr;
}

/*
  ==============================
  U_RingGetCurrentNext

  ==============================
*/
void * U_RingGetCurrentNext( u_ring_t *ring )
{
	if ( !ring->current )
		return NULL;
	return ring->current->next->objptr;
}
...
void * U_RingRemoveGoNext( u_ring_t *ring )
{
	...
	obj = ring->current->objptr;
	next = ring->current->next;

	if ( next == ring->current )
	{
		// last node
		U_FreeDLN( ring->current );
		ring->current = NULL;
	}
	else
	{
...
void * U_RingRemoveGoPrev( u_ring_t *ring )
{
	...
	obj = ring->current->objptr;
	prev = ring->current->prev;

	if ( prev == ring->current )
	{
		// last node
		U_FreeDLN( ring->current );
		ring->current = NULL;
...
/*
  ...
  ==============================
*/
void * U_RingSearchObj( u_ring_t *ring, void *objptr )
{
	u_dln_t		*n;
...
/*
  ...
  ==============================
*/
static __inline__ u_avln_t * AVLInsertRecursive( u_avl_tree_t *tree, u_avln_t *n, void *obj, void *key )
{
	int	balance;
	void	*other_key;
	int	cmp;
...
/*
  ...
  ==============================
*/
u_avl_tree_t * _U_NewAVLTree( int (*key_compare_func)(const void *k1, const void *k2), void * (*get_key_func)(const void *obj) )
{
	u_avl_tree_t	*tree;
...
/*
  ...
  ==============================
*/
void  U_InitAVLTree( u_avl_tree_t *tree, int (*key_compare_func)(const void *k1, const void *k2), void * (*get_key_func)(const void *obj) )
{
...
/*
  ...
  ==============================
*/
void * U_AVLTreeGetTopKey( u_avl_tree_t *tree )
{	
	if ( !tree->topnode )
		return NULL;
...
/*
  ...
  ==============================
*/
void U_InitMap( u_map_t *map, mapAlgorithm type, int (*key_compare_func)(const void *k1, const void *k2), void * (*get_key_func)(const void *obj) )
{
	...
	{
		Error( "U_NewMap: map algorithm not supported yet\n" );
	}
}
...
void * U_MapSearch( u_map_t *map, void *key )
{
	if ( map->type == map_linear )
	{
		return MapSearch_linear( map, key );
	}
	else if ( map->type == map_avl_tree )
	{
		return U_AVLTreeSearch( map->a.a_avl_tree.tree, key );
	}
...
void * U_MapRemove( u_map_t *map, void *key )
{
	if ( map->type == map_linear )
	{
		return MapRemove_linear( map, key );
	}
	else if ( map->type == map_avl_tree )
	{
		return U_AVLTreeRemove( map->a.a_avl_tree.tree, key );
	}
...
/*
  ...
  ==============================
*/
static u_list_t *g_obj_list;

static void MapGetObjList_CB( void *obj )
{
	// keep order of objects
	U_ListInsertAtTail( g_obj_list, obj );
}

void U_MapGetObjList( u_map_t *map, u_list_t *obj_list )
{
	g_obj_list = obj_list;
	U_MapForEach( map, MapGetObjList_CB );
}
