...
void BuildMapNodesRecursive( hobj_t *mapnode, hobj_t *bspnode, hmanager_t *visleafhm )
{
	...
	if ( pair )
	{
		...
		if ( pair )
		{
			...
			// copy clsref for debug draw of visleafs
			...
			// copy center
			pair = FindHPair( visleaf, "center" );
			if ( !pair )
				Error( "missing 'center' in visleaf '%s'.\n", bspnode->name );
			pair = NewHPair2( pair->type, pair->key, pair->value );
			InsertHPair( mapnode, pair );

			// copy bound min
			pair = FindHPair( visleaf, "min" );
			if ( !pair )
				Error( "missing 'min' in visleaf '%s'.\n", bspnode->name );
			pair = NewHPair2( pair->type, pair->key, pair->value );
			InsertHPair( mapnode, pair );

			// copy bound max
			pair = FindHPair( visleaf, "max" );
			if ( !pair )
				Error( "missing 'max' in visleaf '%s'.\n", bspnode->name );
			pair = NewHPair2( pair->type, pair->key, pair->value );
			InsertHPair( mapnode, pair );
