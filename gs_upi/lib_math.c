...
libMathNormType LibMath_GetNormType( vec3d_t norm )
{
	...
	{
		flags = libMathNormType_x;

		if ( norm[0] > 0.0 )
			flags |= libMathNormType_pos;
		if ( an[0] == 1.0 )
			flags |= libMathNormType_exact;
		
	}
	else if ( an[1] >= an[0] && an[1] >= an[2] )
	{
		flags = libMathNormType_y;
...
void Vec3dProjectOrthoOnPlane( vec3d_t out, vec2d_t in, vec3d_t norm, fp_t dist, libMathNormType type )
{
	...
	if ( (type&libMathNormType_axis_mask) == libMathNormType_z )
	{
		Vec3dInit( p1, in[0], in[1], 8192.0 );
		Vec3dInit( p2, in[0], in[1], -8192.0 );
	}
	else if ( (type&libMathNormType_axis_mask) == libMathNormType_x )
	{
		Vec3dInit( p1, 8192.0, in[1], in[0] );
		Vec3dInit( p2, -8192.0, in[1], in[0] );
	}
	else if ( (type&libMathNormType_axis_mask) == libMathNormType_y )
	{
		Vec3dInit( p1, in[0], 8192.0, in[1] );
		Vec3dInit( p2, in[0], -8192.0, in[1] );
	}
	
	d1 = Vec3dDotProduct( p1, norm ) - dist;
	d2 = Vec3dDotProduct( p2, norm ) - dist;
	scale = d1/(d1-d2);
	out[0] = p1[0] + (p2[0]-p1[0])*scale;
	out[1] = p1[1] + (p2[1]-p1[1])*scale;
	out[2] = p1[2] + (p2[2]-p1[2])*scale;
